pipeline {
  ...
  environment {
    ...
    // TAG_NAME = "${!params.TAG_NAME ? (new Date()).format('yyyyMMddHHmm') : params.TAG_NAME}"
    // IMAGE = "$REGISTRY/$DOCKERHUB_NAMESPACE/$MODULE_NAME:$BRANCH_NAME-$BUILD_NUMBER-SNAPSHOT"
    ...
    stage('构建并推送快照镜像') {
      steps {
        ...
        container('maven') {
          ...
          sh 'cd $MODULE_NAME && docker build -f k8s/Dockerfile -t $IMAGE .'
      ...
      steps {
        ...
        container ('maven') {
          withCredentials([
              ...
              sh 'cd $MODULE_NAME && envsubst < k8s/k8s-deploy.yaml | kubectl apply -f -'
              }
        }
      }
    }
